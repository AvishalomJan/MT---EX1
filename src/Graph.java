...
public class Graph {
    ...
    private Bag<DirectedEdge>[] neighbors;    // adj[v] = adjacency list for vertex v
    ...
    public Graph(int V) {
        ...
        neighbors = (Bag<DirectedEdge>[]) new Bag[V];
        for (int v = 0; v < V; v++)
            neighbors[v] = new Bag<DirectedEdge>();
    }
    ...
    public void AddBlackList(int[] blackList) {

        for (int i = 0; i < blackList.length; i++) {
            Bag B = neighbors[blackList[i]];
            Iterator It = B.iterator();
            while (It.hasNext()) {
                DirectedEdge directedEdge = (DirectedEdge) It.next();
                queue.add(directedEdge.weight());
                directedEdge.setWeight(100000);
